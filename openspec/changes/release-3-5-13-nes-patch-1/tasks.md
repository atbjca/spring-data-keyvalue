...
- [ ] 4.1 Stage only files covered by this OpenSpec, including required component release documentation.
- [ ] 4.2 Inspect the staged diff and verify the target version, internal RELEASE dependencies, exclusions, and absence of unrelated files or secrets.
- [ ] 4.3 Create the dedicated release commit for `spring-data-keyvalue-3.5` `3.5.13-nes.patch.1`.
...
- [ ] 7.1 Confirm `v3.5.13-nes.patch.1` does not already exist locally or remotely and the target release commit SHA is unchanged.
- [ ] 7.2 Create annotated tag `v3.5.13-nes.patch.1` on the exact release commit only after `nexus-verified`.
