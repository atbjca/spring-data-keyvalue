...
- [x] 1.1 Confirm this change is apply-ready and assigned exclusively to `spring-data-keyvalue-2.7` in `spring-data-keyvalue-2.7`.
- [x] 1.2 Acquire the component owner lease and record owner/session/timestamps in the release manifest.
- [x] 1.3 Record branch, `origin` fetch/push URLs, HEAD SHA, active OpenSpec changes, toolchain, current version, and target `2.7.18-nes.patch.1` without exposing credentials.
...
- [x] 1.5 Confirm all declared upstream components are Nexus-verified RELEASEs and record their evidence: `spring-framework-5.3`, `spring-data-commons-2.7`.
...
- [x] 2.1 Set the component version to `2.7.18-nes.patch.1`.
...
- [x] 2.3 Discover the complete local publication set and reconcile it with the representative GAVs: `cn.bjca.footstone.bpring.data:bjca-footstone-bpring-data-keyvalue:2.7.18-nes.patch.1`.
...
- [x] 3.2 Audit and reuse the committed 285-test evidence under the same exception; do not rerun `make test`.
...
- [x] 4.3 Create the dedicated release commit for `spring-data-keyvalue-2.7` `2.7.18-nes.patch.1`.
...
- [x] 7.1 Confirm `v2.7.18-nes.patch.1` did not already exist locally and the target release commit SHA is unchanged; remote check remains blocked by GitHub connectivity.
- [x] 7.2 Create annotated tag `v2.7.18-nes.patch.1` on the exact release commit only after `nexus-verified`.
